...
function init() {
            ...
            onBtnClick: function () {
                ...
                if (!errors.length) {
                    ...
                    if (!currentMarkerId){
                        ...
                        let geoObject = objectManager.objects.getById(currentMarkerId);
                        let newComment = {
                            name: form.elements.name.value,
                            place: form.elements.place.value,
                            comment: form.elements.comment.value,
                        ...
                        geoObject.properties.comments.unshift(newComment);

                        console.log(currentMarkerId);
                        console.log(geoObject);
                    }

                    commentHtml = buildCommentHtml(form.elements.name.value, form.elements.place.value, form.elements.comment.value, date);
    ...
    * */
    let ClusterItemContentLayout = ymaps.templateLayoutFactory.createClass(
        '<h2 class="cluster-header">{{ comment.place }}</h2>' +
    ...
    if(data.length !== 0){
        for(let i = 0; i < data.length; i++){
            markersObjects.push({
                ...
                options: {
                    preset: data[i].options.preset,
                },
                properties: {
                    comments: data[i].comments,
                    address: data[i].address,
                    coords: data[i].coords,
                    iconContent: data[i].iconContent
                }
            })
        }
    }
    ...
    * */
    ...
    objectManager.objects.events.add('balloonopen', e => {
        currentMarkerId = e.get('objectId');
